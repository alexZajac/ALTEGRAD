...
d = 1
model = GNN(d, n_hidden_1, n_hidden_2, n_hidden_3, n_class, device).to(device)
...
for epoch in range(epochs):
    ...
    for i in range(0, N_train, batch_size):
        ...
        adj_batch = sp.block_diag(
            [
                nx.adjacency_matrix(g) + sp.identity(g.number_of_nodes())
        ...
        graph_node_counts = [
            g.number_of_nodes() for g in batch_graphs
        ]
        features_batch = torch.ones(sum(graph_node_counts), d)

        for j, n in enumerate(graph_node_counts):
            idx_batch.extend([j] * n)
